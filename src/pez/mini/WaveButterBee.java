...
// WaveButterBee, by PEZ. 0-D DC (recency only) GF gun + surf.

public class WaveButterBee extends AdvancedRobot {
	static final int FACTORS = 25;
	static final int MIDDLE_FACTOR = (FACTORS - 1) / 2;
	static final double BULLET_POWER = 1.9;
	static final double WALL_MARGIN = 18;

	static WaveButterBee robot;
	...
	public void onScannedRobot(ScannedRobotEvent e) {
		...
		if (energyDelta > 0 && energyDelta <= 3) {
			...
			sw.bearingDirection = bearingDir(robotVelocity, getHeadingRadians(), enemyAbsoluteBearing + Math.PI);
		...
		if (enemyVelocity != 0) {
			enemyBearingDirection = bearingDir(enemyVelocity, e.getHeadingRadians(), enemyAbsoluteBearing);
		}
	...
	static double bearingDir(double v, double h, double b) {
		return Math.copySign(0.7 / MIDDLE_FACTOR, v * Math.sin(h - b));
	}
