...
public class Aristocles extends AdvancedRobot {
	...
	public void onScannedRobot(ScannedRobotEvent e) {
		...
		wave.bulletPower = Math.min(getEnergy() / 2, Math.min(e.getEnergy() / 4,
				(distanceIndex = (int)(enemyDistance / (MAX_DISTANCE / DISTANCE_INDEXES))) > 1 ? Math.min(MAX_BULLET_POWER, BULLET_POWER + Math.max(0, getEnergy() - e.getEnergy()) * .11) : MAX_BULLET_POWER));
		...
		public boolean test() {
			if ((distanceFromGun += bulletVelocity(bulletPower)) > gunLocation.distance(enemyLocation)) {
				...
				for (int s = FACTORS; --s >= 0;) {
					factors[s] += MIDDLE_FACTOR / (Math.abs(gf - s) + 1);
				}
